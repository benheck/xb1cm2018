...
void getTimeDigital() {				//Manual time of flight testing
    ...
    while(!(GPIOD_PDIR & lightBitMask)) {  //Wait for sensor to go dark (high)
		if (!(GPIOA_PDIR & (1 << 12))) {	//USER button can abort out of the wait loop
			break;
		}
    }
	...
	//If we aborted using USER button wait here to avoid debounce
    debounceTimer = 50;
    aState = 10;
...
void checkCommand() {
  ...
  if (op0 == '/') {       //Special debugging commands (more than 2 characters)
    ...
    if (op1 > 96) {       //Convert lower to uppercase
      op1 -= 32;      
    }
...
void sendPots() {			//Sends raw data to pots (no ranging)
  ...
  Wire.write(B00000000);            //Write to LT 
  Wire.write(map(LT, LTp, LTr, triggerRangeLL, triggerRangeLH)); //The scaling to override the Hall Effect Sensors is far different than the analog pots!
  Wire.write(B00010000);            //Write to RT
  Wire.write(map(RT, RTp, RTr, triggerRangeRL, triggerRangeRH));      
  Wire.endTransmission();     // stop transmitting  
...
void rangingPot() {		//Get a value from UART and stores it in the analog pot ranging variables / EEPROM
    ...
    if (fileString[x] > 96) {         //Convert upper to lowercase
      fileString[x] -= 32;
    }
	...
	switch(fileString[0]) {
      ...
      if (fileString[1] == 'T') {       //Trigger range command?
        
        if (fileString[2] == 'H') {
          triggerRangeRH = value;
          Serial.print("RTH=");
          Serial.println(value);
        }
        else {
          triggerRangeRL = value;
          Serial.print("RTL=");
          Serial.println(value);					
        }      
      ...
      if (fileString[1] == 'T') {       //Trigger range command?
        ...
        if (fileString[2] == 'H') {
          triggerRangeLH = value;
          Serial.print("LTH=");
          Serial.println(value);
        }
        else {
          triggerRangeLL = value;
          Serial.print("LTL=");
          Serial.println(value);					
        }      
		...
		default:
			...
			Serial.print("LTL=");
			Serial.print(triggerRangeLL);
			Serial.print("\tLTH=");
			Serial.println(triggerRangeLH);
			Serial.print("RTL=");
			Serial.print(triggerRangeRL);
			Serial.print("\tRTH=");
			Serial.println(triggerRangeRH);  
